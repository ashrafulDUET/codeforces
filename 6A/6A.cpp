...
int main()
{
   FastRead
    ...
    while(cin>>a>>b>>c>>d)
    {
        if(isTri(a,b,c) or isTri(a,b,d) or isTri(b,c,d) or isTri(c,d,a))
            cout<<"TRIANGLE"<<endl;
        else if(isSeg(a,b,c) or isSeg(a,b,d) or isSeg(b,c,d) or isSeg(c,d,a))
            cout<<"SEGMENT"<<endl;
        else
